    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            for candidate in candidate_units:
                ...
                if candidate in reference_task_tree:
                    continue
                
                mcts_task_tree= MCTS(goal_node, kitchen_items, utensils, motion_nodes, foon_object_nodes, foon_object_to_FU_map, foon_functional_units, candidate)
                # Initialize MCTS with goal node, kitchen items, functional units, and motion success rates

                # Run the MCTS algorithm for a defined number of iterations and simulate k executions for each FU
                current_success=mcts_task_tree.run(iterations=1000, k=10)  # Simulate 10 random executions for each functional unit (FU)

                # selecting functional unit with max success
                if current_success>max_fu_success:
                    max_fu_success=current_success
                    selected_candidate_idx=candidate
            
            if selected_candidate_idx is None or selected_candidate_idx in reference_task_tree:
                continue
    ...
    with open("motion.txt") as file:
        for line in file:
            ...
            if len(parts) >= 2:
                ...
                print(f"Ignoring line: {line.strip()} (doesn't have at least 2 values)")
